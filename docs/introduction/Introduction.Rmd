...
author: "Kyle Hardman"
date: "Package version 0.7.5, May 2017"
output: pdf_document
linkcolor: blue
...
I consider these plots to be publication quality (I used them in an article) and would recommend that you consider publishing such plots as well, rather than tables or omitting parameter estimates altogether.

To plot individual panels of the larger figure, you can use some lower-level plotting functions, including `plotCatMu` for the `catMu` parameter, `plotHistWithMean` for histograms for parameters without condition effects, and `plotPosteriorLineChart` or `plotFactorialPosteriorLineChart` for single-factor or factorial designs for which there are condition effects.
...
Note that the test is done in each condition individually. The null hypothesis for each test is that the parameter value is equal to the null hypothesized values, `pContBetween_test` and `pCatGuess_test`.
...
+ `pBetween` ($P^B$): The probability that a memory response is a between-item response. This wasn't actually in the article.
+ `pContBetween` ($P^O$): The probability that a between-item memory response is continuous in nature.
+ `pContWithin` (also $P^O$): The proportion of a within-item memory response that is continuous in nature. In future articles, I would probably use $P^{OW}$ for `pContWithin` and $P^{OB}$ for `pContBetween`.
...
The condition effect parameters account for differences in participant-level parameters between task conditions. The priors on the condition effect parameters are Cauchy distributions with location 0 and some scale value. In particular, the default scales are `r pr$pMem_cond.scale` for the probability parameters (e.g. `pMem`, `pContBetween`) and `r pr$contSD_cond.scale` for the standard deviation parameters (e.g. `contSD`, `catSelectivity`). Note that the scale of the parameters should be thought of with respect to the untransformed space. see the Condition Effects section of the Appendix of @HardmanVergauweRicker2017 for information about the transformation.
