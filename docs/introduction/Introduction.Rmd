...
author: "Kyle Hardman"
date: "Package version 0.7.5, April 2017"
output: pdf_document
linkcolor: blue
...
There are two basic steps to using this package: 1) Running the parameter estimation and 2) analyzing the posterior distributions of the parameters. These steps each have many sub-steps, which are described in detail in the following sections.
...
The models are designed for within-participants designs in which all participants have data in all conditions. This allows for maximal interpretability of the condition effect parameters. 

With a between-participants design, the model will run, but it is possible to have differences between conditions get soaked up, at least in part, by participant level parameters due to different participants being in different conditions. In theory, this is mitigated by the fact that there is a hierarchical prior on all of the participant level parameters. This should keep all participants somewhat clustered around one population grand mean, while the condition effects account for differences between conditions, but this is only in theory. 

In practice I have found that fully between-participants designs do not work properly. What happens is that the condition effect parameters trade off badly with participant level parameters, making inference about parameters questionable.

I have experimented with a mixed between/within design in which all participants do 2 conditions out of 3 possible conditions. Making there be some overlap between participants and conditions seems to help and the model seems to work acceptably well.

If you have a fully between-participants design, you are not totally out of luck. You should fit the model separately for each group of participants. Then you should contact me and I may be able to help you. I am working on adding some support for between-participants designs to the package and I may have the piece you need ready.
...
If you want to calculate WAIC for a model that you made, i.e. one not from the CatContModel package, you should be aware that the magnitude of densities are different depending on whether you are working in degrees or radians. This affects the validity of comparing WAIC values between models that work in degrees vs radians. 
...
Working Memory Models for Delayed Estimation Tasks (Version 0.7.5) [Computer software]. Retrieved from
https://github.com/hardmanko/CatContModel/releases/tag/v0.7.5
...
This was the best model selected by @HardmanVergauweRicker2017. If you are going to use one categorical model from this package for color data, you should use this model. However, you could try fitting more than one model and comparing the fit of those models with [WAIC](#WAIC).
