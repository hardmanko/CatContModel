...
author: "Kyle Hardman"
date: "Package version 0.8.1, October 2018"
output: pdf_document
number_sections: yes
linkcolor: blue
...
Hardman, K.O. (2018). CatContModel: Categorical and Continuous 
...
`catSelectivity` is really hard to estimate. Don't expect to be able to do anything fancy with it, like giving it a condition effect as discussed in the [Constraining Parameter Estimation](#constrainingParameterEstimation) section. My recommendation is to just use one `catSelectivity` parameter per experiment and to not use it for inference. It could be a parameter of interest for some studies, but the difficulty in estimating `catSelectivity` means that I have to recommend against trying such a study.
