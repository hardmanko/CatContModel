...
author: "Kyle Hardman"
date: "Package version 0.9.0, November 2022"
output: pdf_document
number_sections: yes
linkcolor: blue
...
The data must be stored in a `data.frame` where each row of the data frame has the data for a single study/response pair, with information about which participant and condition the study/response pair are from. The data should have the following 4 columns, in no particular order:

1. `pnum`: Character (or convertible to character). Participant number/identifier. You can use any number of participants.
2. `cond`: Character (or convertible to character). The names task conditions. If you only have 1 condition, just give any constant value here.
...
setwd("D:/Programming/R/CatContModel/docs/introduction/")
```
```{r}
...
The data may have additional columns, like trial number, etc. The package will ignore extra columns without complaint.

## Main Parameter Estimation 0.9.0

As of version 0.9.0, parameter estimation is more feature rich, but also more complex.

```{r eval=FALSE}
config = makeModelConfig(data, modelVariant="betweenItem")

results = runParameterEstimation(data, config, iterations=500)
```
...
parCfg = makeParallelConfig(numNodes = 2, outputFile="parallelProgress.txt")

mhOptimCfg = makeMHConfig(keepOptimIter = TRUE)

results = runParameterEstimation(data, config, iterations=500, parallelConfig=parCfg, mhOptim=mhOptimCfg)

```
```{r eval=FALSE}
saveRDS(results, "BI_parallel_500.RDS")
```
...
```{r eval=FALSE}
convergencePlots(results, filename="convergence.pdf")
```
...
ppHist(0, 1.0^2, 0.5, 0.5, type = "prob", lpos = "top")
...
#### Choosing Condition Effect Priors

Choosing condition effect parameter priors is somewhat complicated by the fact that the condition effects parameters are in the latent space, but you really want to know what the prior is in the manifest space.

For probability parameters, the logit transformation is used to travel between latent and manifest spaces.
Due to the nonlinearity of the logit transformation, the magnitude of the condition effect in the manifest space depends on the magnitude of the participant parameters that the condition effects are added to.
...
# logit(0.5) converts the manifest probability 0.5 to the latent space value (which happens to be 0)
probParamManifestConditionEffect(logit(0.5), 1)
probParamManifestConditionEffect(logit(0.7), 1)
```

As you can see, the manifest condition effect is larger when the participant parameter is 0.5 than when it is 0.7. In general, the manifest condition effects become smaller as the participant parameter gets farther from 0.5. This is due to the particular nonlinear behavior of the logistic transformation
...
probParameterManifestConditionEffectPlot = function(p_man, ce_scale, n=1e6) {
	ce = rcauchy(n, 0, ce_scale)
	p = probParamManifestConditionEffect(logit(p_man), ce)
	hist(p, prob=TRUE, xlab="Manifest CE",
			 main = paste0("p = ", p_man, ", scale = ", ce_scale))
}

par(mfrow=c(3,2))

probParameterManifestConditionEffectPlot(0.5, 0.3)
probParameterManifestConditionEffectPlot(0.8, 0.3)
probParameterManifestConditionEffectPlot(0.5, pr$pMem_cond.scale)
probParameterManifestConditionEffectPlot(0.8, pr$pMem_cond.scale)
probParameterManifestConditionEffectPlot(0.5, 3)
probParameterManifestConditionEffectPlot(0.8, 3)
```

In the title of each plot, `p` is the latent participant parameter value and `scale` is the prior scale of the Cauchy distribution from which prior condition effects are sampled. The x-axis of each plot is the condition effect in the manifest (probability) space, so 0.2 indicates a 20% difference between conditions.

What you can see is that as the scale increases, the prior becomes increasingly heavy at the ends, which is probably not a good prior for many typical cases. You probably would not expect so much mass at such large differences between conditions of most designs as when the scale is 3.

Another way of looking at the priors is to calculate a prior credible interval: The endpoints of the central 95\% of the prior.

```{r fig.height=3}
ps = seq(0, 1, 0.025)

conditionEffectPriorCredibleInterval("pMem", ps, 0.1)
conditionEffectPriorCredibleInterval("pMem", ps, pr$pMem_cond.scale)
conditionEffectPriorCredibleInterval("pMem", ps, 0.5)
```

Turning to the SD parameters, the process works similarly but is a little more straightforward. In particular, as you can see from the code below, a latent condition effect of 3 results in a manifest condition effect of 3 for participant parameter values of both 10 and 15. This is true except when the latent condition effect is negative and results in a combined latent value of less than the minimum standard deviation cutoff (which is 1 by default; set by `config$minSD`).

```{r}
sdParamManifestConditionEffect = function(sd_lat, ce_lat) {
	trans = function(x) {
		pmax(x, 1)
	}
	noCE = trans(sd_lat)
	withCE = trans(sd_lat + ce_lat)
...
To examine distributions for standard deviation parameters, we can use the same process as with probability parameters. The one modification is to cut off the plots above 50 so that we can see the lower part of the distribution better.
...
The spikes at the left side of the distributions are due to the transformation of SD parameters cutting off the distribution below a point and pushing all of the mass below that point up to the cutoff point.
...
# DO NOT RENAME THE CELL IN THE cond COLUMN. Names in cond relate to the data and cannot change.
...
Hardman, K.O. (2022). CatContModel: Categorical and Continuous 
Working Memory Models for Delayed Estimation Tasks (Version 0.8.1) [Computer software]. Retrieved from
https://github.com/hardmanko/CatContModel/releases/tag/v0.8.1
...
Parameter estimation for the within-item model is somewhat fragile. The model may not work correctly in the following situations, which are elaborated on in the following subsections.

1) The number of categories is low (3 or less).
2) The data contains very little categorical guessing.

#### Low Number of Categories

The within-item model does not work well with a low number of categories. When the number of categories is low (approximately 1 to 3), the model will likely estimate that `pContWithin` is nearly 1.

The left panel below shows a case with a single category at 180 degrees. 
Other than the categorical guessing band, responses are continuous memory responses.
...
study = runif(1000, 0, 360) # Sample 1000 study values

# Panel 1
param = list(pMem = 0.7, pContWithin = 1, pCatGuess=1, contSD=20, catSD=10, catSelectivity=10, catMu = 180)

samp = sampleDataFromModel(study, param, "withinItem")

scatterplotWithColorBars(samp)
abline(0,1)
title("Data with 1 category")

# Panel 2
param$pContWithin = 0.5

#param = list(pMem = 0.7, pContWithin = 0.5, pCatGuess=0.8, contSD=20, catSD=10, catSelectivity=10, catMu = 180)

samp = sampleDataFromModel(study, param, "withinItem")

scatterplotWithColorBars(samp)
abline(0,1)
title("Model with pContWithin=0.5")


# Panel 3
param$catMu = c(90, 270)
param$pMem = 1

#param = list(pMem = 0.7, pContWithin = 0.5, pCatGuess=0.8, contSD=20, catSD=10, catSelectivity=10, catMu = c(90, 270))
...
If the within-item model were used with the data, it would find the single category at 180 degrees. 
If the model were to try the value 0.5 for `pContWithin, the resulting model prediction is that memory responses would
be on a line going from (0, 90) to (360, 270), as in the right panel.
The model prediction in the right panel is very dissimilar from the data, so a value of 0.5 for `pContWithin` does not work well, but a value of 1 would work well (as in the left panel).


With 2 categories, the problem is less severe, but still present. If `pContWithin` is less than 1, the model predictions move away from the line of ideal responding. 

With few categories, the line segments of modeled memory responses are long. As `pContWithin` reduces from 1, the ends of the line segments move far from the line of ideal responding, which misses most of the data (in typical data sets).

With more categories in the data, the within-item model uses multiple line segments, so no part of any line segment is too far from the intercept 0, slope 1 line. As the number of categories increases, `pContWithin` is able to take values less than 1 more easily.



These plots show data sampled from a within-item model with 1 category at 180 degrees (left) or 2 categories at 90 and 270 degrees (right). This is 

```{r}

param = list(pMem = 1, pContWithin = 0.5, pCatGuess=0.8, contSD=20, catSD=10, catSelectivity=10, catMu = c(90, 270))

samp = sampleDataFromModel(runif(1000, 0, 360), param, "withinItem")
...
```
 ...
 Imagine the case in which there is only 1 active category located at 180 degrees and that `pContWithin` is 0.5. This means that the model will predict that memory responses will be on a line essentially going from (0, 90) to (360, 270), passing through (180, 180). This line would provide terrible fit for any normal data. This forces `pContWithin` to 1 so that the line will go from (0, 0) to (360, 360) and capture the bulk of the data. With more active categories, it becomes less of an issue as there are multiple line segments, so no part of any line segment is too far from the intercept 0, slope 1 line.

The between-item model doesn't have this issue because the categorical and continuous responses are separate. Continuous responses always follow the line of ideal responding in that model


Solution 2: Force some categories to be active. You can use constant parameter values overrides to force a few categories (maybe 4) to always be active, but that does have the effect of always having a few active categories. See the `setConstantCategoryParameters` function.



#### Very Little or No Categorical Guessing

For the within-item model, it is difficult to properly estimate `contSD` when there is very little or no categorical responding in the data.

The within-item model assumes that each memory response is a mix of 1) continuous information about the exact value of the studied stimulus and 2) categorical information about the category of the studied stimulus. The variability of memory responses ($\sigma^{W}$ or `withinSD`) is a mixture of continuous variability ($\sigma^{O}$ or `contSD`) and categorical variability ($\sigma^{A}$ or `catSD`).

By default, $\sigma^{W}$ is not estimated directly, but is a function of `contSD`, `catSD`, and `pContWithin`. The function for $\sigma^{W}$ is given in Equation 25 of @HardmanVergauweRicker2017, reproduced here with simplified notation where $P$ is `pContWithin`.

$\sigma^W = [ P^2 * (\sigma^O)^2 + (1 - P)^2 * (\sigma^A)^2 ]^{1/2}$

As shown in Equation 22 of @HardmanVergauweRicker2017, $\sigma^W$ is used to calculate the likelihood of memory responses. 
Thus, $\sigma^{W}$ is straightforward to estimate because it is based directly on data, but `contSD` and `catSD` cannot be estimated directly: they could trade off perfectly without some other constraint. That other constraint comes from the estimation of `catSD` from categorical guesses.

$\sigma^O$ and $\sigma^A$ are part of the calculation for $\sigma^W$, but are not themselves strongly identified from memory responses. $\sigma^O$ and $\sigma^A$ can trade off with one another (negative cross-correlation) and still produce the same value for $\sigma^W$. 

The constraint on estimation of `contSD` and `catSD` comes from the estimation of `catSD` from categorical guesses.

For the within-item model, `contSD` is estimated from the memory responses, which have both a categorical and a continuous component. `catSD` is estimated from categorical guesses, but also from the same memory responses as `contSD`. Thus, `catSD` can be estimated accurately based on the categorical guessing bands, which allows `contSD` to be estimated from the memory responses given that `catSD` has been estimated from the categorical guesses.

**Conclusion**: From the above, if there is no categorical guessing, the within-item model _cannot estimate_ either `catSD` and `contSD`. Those parameters will have substantial negative cross-correlations with one another (at the participant level in the model). If there is very little categorical guessing (maybe 10\% of guesses), the model will not reliably estimate either `catSD` or `contSD` with default settings (but see Solutions below).


Note: The between-item model does not have a problem with low amounts of categorical guessing. In that model, `catSD` is estimated from categorical memory responses and categorical guesses. If there are no categorical guesses, `catSD` can be estimated from categorical memory responses. Also, `contSD` is estimated from continuous memory responses without any connection to the value of `catSD`.



*Solution 1*: If there is truly no categorical guessing in your data, one partial solution would be to set `catSD` ($\sigma^A$) to the constant value 0 (and probably also set `pCatGuess` to 0 to be sure that no categorical guessing is estimated). In this case, you cannot estimate both $\sigma^O$ and $\sigma^A$, but you can estimate their mixture, $\sigma^W$. The model doesn't directly estimate $\sigma^W$, but you can calculate it after the fact by using the following logic.

Start from Equation 25 and set $\sigma^A = 0$, which results in the following simplification:
...
*Solution 2*: If you have a small but decidedly nonzero amount of categorical guessing, it may work to set `catSD` to a constant value. Based on past research you may be able to select a reasonable constant value.

*Solution 3*: A new feature in v0.9.0 is to estimate $\sigma^{W}$ independently of `catSD`. By setting `withinItem_contSD_is_withinSD = TRUE` in the model configuration (see `makeModelConfig` function), `withinSD` is directly estimated rather than being indirectly estimated through other model parameters. Note that there will not be a parameter named `withinSD`: you must remember your setting and interpret the parameter named `contSD` as `withinSD`.
...
In order to accommodate linear data, a few modifications to the distributions used by the model were required. With linear data, rather than using Von Mises distributions, the model uses a combination of normal and truncated normal distributions. With reference to the equation numbers in the Appendix of @HardmanVergauweRicker2017, the following modifications were made:
...
This code example only shows `contSD`, but the logic is easily extended to the other standard deviation parameters. See the Choosing Priors subsection within the [Priors](#priors) section for more information on how to select priors.

### Category Location Restriction

Currently, the `catMu` parameters are not allowed outside of the range of the data. This may change at some point once I make a difficult decision about how to allow it to happen. Details: There is a calculation in the likelihood function in which the density of an observed response given a `catMu` parameter is calculated. The density function is a truncated normal. If the `catMu` parameter is far outside of the possible range of responses, the proportion of the normal distribution within the response range (which is the truncation scale factor) is approximately 0. To calculate the truncated density, a normal density is divided by the truncation scale factor. Division by 0 is bad, so I have to choose how to prevent division by 0. There are no good solutions (at least that I know of), only less-bad solutions. The current less-bad solution is to prevent the `catMu` parameters from being outside of the response range. Obviously, the internal categories used by participants do not necessarily map neatly onto the response space chosen by the experimenter. It is always possible that there is a category just outside of the response space but that category is forced into the response space by the model. This is a good reason to make the response range wider than the range of studied values in the experiment: If a participant has a category outside of the range of studied values but within the response range, the data will show their responses at the category location.


## catSelectivity

`catSelectivity` is really hard to estimate. Don't expect to be able to do anything fancy with it, like giving it a condition effect as discussed in the [Constraining Parameter Estimation](#constrainingParameterEstimation) section. My recommendation is to just use one `catSelectivity` parameter per participant and to not use it for inference. It could be a parameter of interest for some studies, but the difficulty in estimating `catSelectivity` means that I have to recommend against trying such a study.
