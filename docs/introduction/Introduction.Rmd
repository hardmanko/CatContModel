...
If working with parameters directly, it is extremely important that you remember that most of the parameters exist in a latent space. For example, the probability parameters exist on the interval `(-Inf, Inf)` and are transformed to probabilities in the interval $[0,1]$ with the logit transformation. 
...
Note that inactive categories have already been removed from `param` which is why it does not include the `catActive` parameters (but see the `removeInactiveCategories` argument).
...
A simple usage of each of the helper funcions is shown below. See the documentation of those functions for more information on usage. You should 
...
Working Memory Models for Delayed Estimation Tasks (Version 0.6.1) [Computer software]. Retrieved from
https://github.com/hardmanko/CatContModel/releases/tag/v0.6.1
...
# Use continuous SD with mean 5 and variance 225.
...
results = runParameterEstimation(config, data, priorOverrides = priorOverrides, mhTuningOverrides = mhTuningOverrides)
```
...
This package has some support for contraining parameter estimation and performing hypothesis tests for factorial designs. For now, only two fully-crossed factors are fully supported. I think that non-fully-crossed factors may be ok, but I haven't gone through the logic of it yet. Designs with three or more factors are only partially supported, so I would recommend only using two factors.
...
As can be seen in this example, condition "a3" is associated with level "a" of the letters factor and level "3" of the numbers factor. This information is needed by the package so it can know what conditions go with what factor levels. If you have only one factor, you do not need to provide the factors data frame. It is not necessary to include the factors in the data set: You still only need the `cond` column in the data.
...
`catSelectivity` is really hard to estimate. Don't expect to be able to do anything fancy with it, like giving it a condition effect.
