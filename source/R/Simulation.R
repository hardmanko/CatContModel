  ...
  condNames = conditionEffects$condNames
	...
	for (pn in 1:nPart) {
		for (cond in 1:nCond) {
			...
			param = combinedParam[[ condNames[cond] ]][[pn]]
			
			studyValues = stats::runif(trials, studyRange[1], studyRange[2])
			
			samp = sampleDataFromModel(studyValues, param, 
	...
	for (pnum in 1:nPart) {
	  for (condName in names(paramList)) {
			...
			for (n in names(pdata)) {
				...
				temp = data.frame(pnum = pnum, cond = condName, param = n, cat = NA, value = pdata[[n]], stringsAsFactors = FALSE)
				if (n == "catMu") {
...
SIM_prepareTrueRecParam = function(results, trueParam) {
  ...
  commonColumns = c("Reality", "param", "pnum", "cond", "value")
...
SIM_calculateRecoveryStats = function(results, trueParam, roundDigits=NULL) {
  ...
  commonParameters = intersect( unique(prepTrue$param), unique(prepRec$param) )
  
  for (param in commonParameters) {
    
    conds = unique(prepTrue$cond[ prepTrue$param == param ])
    
    for (cond in conds) {
      
      rec = prepRec$value[ prepRec$param == param & prepRec$cond == cond ]
      true = prepTrue$value[ prepTrue$param == param & prepTrue$cond == cond ]
      
      difs = rec - true
      absDifs = abs(difs)
      
      temp = data.frame(param = param, cond = cond, stringsAsFactors = FALSE)
...
#' The `param`, `cond`, and `pnum` columns identify the parameter being tested.
...
SIM_testCredibleIntervals = function(results, trueParam, cip=0.95, roundDigits=NULL) {
  ...
  for (param in paramNames) {
    ...
    for (cond in unique(results$data$cond)) {
      ...
      true.cond = trueParam[ trueParam$param == param & trueParam$cond == cond, ]
      ...
      credInt = getParameterCredibleInterval(results, param, pnum=NULL, cond=cond, cip=cip)

      trueMean = mean(true.cond$value)
      
      temp = data.frame(param = param, cond=cond, pnum=".mu", true=trueMean, ciLower=credInt[1], ciUpper=credInt[2])
      allCI = rbind(allCI, temp)
      
      for (pnum in pnumsInCond) {
        
        credInt = getParameterCredibleInterval(results, param, pnum=pnum, cond=cond, cip=cip)
        
        true.part = true.cond$value[ true.cond$pnum == pnum ]
        
        temp = data.frame(param = param, cond=cond, pnum=pnum, true=true.part, ciLower=credInt[1], ciUpper=credInt[2])
        allCI = rbind(allCI, temp)
      }
  ...
  partContains = stats::aggregate(ciContainsTrue ~ cond * param, justPart, mean)
...
SIM_parameterRecoveryPlots = function(results, trueParam, pdfFile=NULL) {
  ...
  for (param in commonParameters) {
    ...
    thisTrue = prepTrue[ prepTrue$param == param, ]
    thisRec = prepRec[ prepRec$param == param, ]
    ...
    if (length(conds) == 1) {
      ...
      LineChart::lineChart(value ~ cond * Reality, tr, errBarType = NULL)
    }
    graphics::title(param)
  ...
  if (results$config$modelVariant != "ZL") {
    ...
    for (pnum in results$pnums) {
      ...
      trueMu = prepTrue$value[ prepTrue$param == "catMu" & prepTrue$pnum == pnum ]
...
SIM_catMuRecovery = function(results, trueParam) {
  ...
  for (pnum in results$pnums) {
    ...
    trueCatMu = trueParam$value[ trueParam$param == "catMu" & trueParam$pnum == pnum ]
